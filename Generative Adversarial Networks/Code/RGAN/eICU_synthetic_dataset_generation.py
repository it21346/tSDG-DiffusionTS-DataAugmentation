...
def train_split_data(X, Y):
    X_train_temp, X_temp, y_train_temp, y_temp = train_test_split(X.values, Y.values, test_size=0.25, random_state=42, shuffle= False)
...
for identifier in identifiers:
    ...
    X = df.drop(columns= ['Date', 'Time', 'RH', 'AH'])
    X = X.iloc[:, : 10]
    Y = df['T']
    X_train, Y_train, X_val, X_test, Y_val, Y_test = train_split_data(X, Y)
