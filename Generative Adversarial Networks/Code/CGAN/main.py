...
def main():
    ...
    if FLAGS.train:
        print('Loading data...\n')
        dataset = dset.MNIST(root = FLAGS.data_dir, download = True, transform = transforms.Compose([
            transforms.Resize(FLAGS.img_size),
            transforms.ToTensor(),
            transforms.Normalize((0.5,), (0.5,))
        ]))
        assert dataset
        dataloader = torch.utils.data.DataLoader(dataset, batch_size = FLAGS.batch_size, num_workers = 4, pin_memory = True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='../Datasets/mnist', help='Directory for dataset.')
