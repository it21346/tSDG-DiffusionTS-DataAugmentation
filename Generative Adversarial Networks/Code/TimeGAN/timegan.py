...
def timegan (ori_data, parameters):
  ...
  num_train_G = parameters['num_train_G'] # number of training for generator per one discriminator training
  embed_iterr = parameters['embed_iterr'] # number of iterations for embedding network training and supervised loss training only
  z_dim        = dim
  gamma        = 1
  ...
  for itt in range(embed_iterr):
    ...
    # Checkpoint
    if itt % 1000 == 0:
      print('step: '+ str(itt) + '/' + str(embed_iterr) + ', e_loss: ' + str(np.round(np.sqrt(step_e_loss),4)) ) 
  ...
  for itt in range(embed_iterr):
    ...
    # Checkpoint
    if itt % 1000 == 0:
      print('step: '+ str(itt)  + '/' + str(embed_iterr) +', s_loss: ' + str(np.round(np.sqrt(step_g_loss_s),4)) )
  ...
  for itt in range(iterations):
    ...
    for kk in range(num_train_G): # originally 2
      # Set mini-batch
      X_mb, T_mb = batch_generator(ori_data, ori_time, batch_size)               
      # Random vector generation
      Z_mb = random_generator(batch_size, z_dim, T_mb, max_seq_len)
