...
def test(opt, ori_data):
    ...
    orig = pd.read_csv('./data/copper_data.csv')
    # print(orig)
    original = np.loadtxt('./data/copper_data.csv', delimiter=",", skiprows=1, encoding='utf-8')
    # original = original[::-1] #Not sure i should be flipping
    denorm_data = MinMaxInverseScaler(gen_data, original)
    # print(denorm_data)
    # denorm_data = denorm_data[::-1]
    denorm_df = pd.DataFrame(denorm_data[0], columns=orig.columns)
    for i in range(1, len(denorm_data)):
        df_slice = pd.DataFrame(denorm_data[i], columns=orig.columns)
        denorm_df = pd.concat([denorm_df, df_slice], ignore_index=True)
    denorm_df.to_csv("./data/synth_copper_data2.csv", index=False)
