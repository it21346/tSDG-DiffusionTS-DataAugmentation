...
"""
In the case of CycleGAN, we have two Generators ( G and F ) and two Discriminators ( Dx and Dy ).
Now lets imagine that X is a horse image and Y is a zebra image.
The architecture goes along the lines -> G generates a zebra image from X, Dy tries to classify the generated image as real or fake zebra. 
Then F generates a horse image from Y (generated image from G) and Dx tries to classify the generated image as real or fake horse.
In addition, they add two additional loss terms, cycle-consistency loss and identity-mapping loss.
The cycle-consistency loss tries to keep a constistency between the original horse image and the genetated through the pipeline horse image (reconstructed). This is done with the L1-loss.
The identity-mapping loss is there for not changing the color basically. It is also done with the L1-loss.
"""
...
class Generator(nn.Module):
    """
    torch.nn.InstanceNorm2d is used instead of torch.nn.BatchNorm2d, because the former normalization layer is more suitable for style transfer
    """
    def __init__(self, channels, num_blocks = 9):
        ...
        # output
        model += [nn.ReflectionPad2d(3), nn.Conv2d(64, self.channels, 7), nn.Tanh()]
